...
- 55 entries with keys `'variant:<familyId>:<slotIndex>'` covering every catalog entry — each mapping to the 1×1 transparent-PNG data URI used as the scaffold-phase placeholder
- 1 terminal fallback entry `'variant:default'` mapping to the same placeholder

The fallback chain for variant sprite resolution SHALL be:

```
variant:<familyId>:<slotIndex>     (always exists after this change)
  → variant:<familyId>:default     (per-family fallback — NOT registered this change, reserved for future)
  → variant:default:<rarity>       (rarity-tier fallback — NOT registered this change, reserved for future)
  → variant:default                (terminal fallback — registered this change)
```

Real pixel-art sprites for the 55 variants are out of scope for this change. A separate follow-up change SHALL replace the placeholders with real assets generated via the codex CLI or Gemini MCP path per `~/.claude/imports/codex_image_gen.md` / `image_gen_routing.md` recipes. Other sprite categories MAY remain on placeholders until their consumers ship.

#### Scenario: Theme pack registers all 55 variant keys

- **WHEN** the developer iterates over all 55 `(familyId, slotIndex)` combinations
- **THEN** for each pair, `SPRITE_MAP['variant:' + familyId + ':' + slotIndex]` SHALL resolve to a non-empty URL
- **AND** the URL MAY be the 1×1 transparent-PNG data URI (acceptable placeholder for this change)
- **AND** the URL SHALL NOT be `undefined` or an empty string

#### Scenario: Terminal fallback exists

- **WHEN** the developer reads `SPRITE_MAP['variant:default']`
- **THEN** the lookup SHALL resolve to a non-empty URL
- **AND** the resolution SHALL be the same placeholder used for per-variant keys this change
