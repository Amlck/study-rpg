...
## 3. Verification
- [x] 3.1 `pnpm -r typecheck` — full repo type-clean (regression guard against unrelated breakage). Initial run surfaced 3 errors in `LeaderboardPage.tsx` (314 / 376 / 508) re missing `'correct'` in `LeaderboardFilter` and `total_correct` on `LeaderboardRow`. **Post-hoc diagnosis (corrected from original archive note): not a real bug — root cause was a stale `packages/core/dist/index.d.ts` in the hotfix worktree (still on the pre-`add-hospital-leaderboard-correct-count-filter` build artifact, because pnpm install on branch switch doesn't auto-rebuild workspace dist).** Verified by `pnpm --filter @study-rpg/core build` → typecheck re-runs clean. The shipped fix is unaffected; prod CI rebuilds core via `prebuild` hook so bundle is correct.
