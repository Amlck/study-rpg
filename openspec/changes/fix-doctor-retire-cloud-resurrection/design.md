...
- 不動 `services/retire.ts` 業務邏輯 — 該檔已正確
...
**Why now**：A 的「升級既有 log table」成本與 B 相當（新表 vs 新 column 都要 migration + RLS + upsert_lww extend），但 A 沒有混版本誤判風險，且把「retirementLog 已經是事實上的不可變紀錄」這個語意明確化。

### Decision 2: retirementLog 的 Dexie primary key 從 `++id`（auto-increment）改成 `doctorId`（string, deterministic）

**選擇**：Dexie **v18 → v19** migration（前作 `tidy-tabs-add-study-stats-medexam2` 已在 2026-05-26 把 schema 推進 v18 加 `dailyStudyLog` table，本 change 不可重用 v18），read 既有 rows → 以 doctorId 為 pk 重寫（duplicate 時保留 retiredAt 最小那筆）。Sync 也用 doctorId 為 pk。

**Critical**：codex audit-content adversarial review (2026-05-26) Attack 2 抓到本 change 初版寫 v17→v18 是 stale target — 既有 v18 用戶不會 re-run upgrade callback，`db.retirementLog.get(doctorId)` 永遠回 undefined → carve-out silently 失效 → 整個 fix 對 v18 用戶等同沒做。任何 review 看到 `version(18).stores({retirementLog: ...})` 都應該立即視為 critical regression。

**Why**：
- doctorId 天然唯一（retire 是 destructive，同一個 doctor 不可能被同裝置 retire 兩次）
- 跨裝置一致：device A 跟 device B 對同一個 doctorId 的 retire 行為要 dedupe 成 cloud 上一筆，auto-incr id 沒有這個性質
- 簡化 adapter `applyToLocal`：直接 `db.retirementLog.put({doctorId, retiredAt, ...})`，無需先找對應 auto-incr id
- 所有現有 read site（`tick.ts` `where('retiredAt').above(...)`、`achievement-stats.ts` `each()`、`HelpMenu.tsx` `clear()/bulkPut()`、`DomainMigrationBanner.tsx` `toArray()`）對 pk 變化都 agnostic
...
.upgrade(async (tx) => {
  ...
  // Dedup by doctorId, keep smallest retiredAt (oldest retire)
  ...
  // After schema rewrite the new pk is doctorId; bulkPut without explicit id
...
Existing v18 schema string (from `tidy-tabs-add-study-stats-medexam2`): `'++id, retiredAt, doctorId'`（retirementLog 從 v9 起 schema 不變）. **New v19 schema string**: `'doctorId, retiredAt, rarity, _updatedAt'`（pk 換、加 `_updatedAt` LWW field、index `rarity` for achievement query 加速）。

### Decision 3: apply 路徑同時用 carve-out + post-pull reconcile

**選擇**：兩道防線。

- **Carve-out**（apply 階段）：`HOSPITAL_DOCTORS.applyToLocal` 第一步檢查 `db.retirementLog.get(cloudRow.id)`，命中即 skip 並 delete local 殘留。
- **Reconcile**（post-pull 階段）：`reconcileRetiredDoctors()` iterate `db.retirementLog`，對每個 `doctorId` 呼叫 `db.doctors.delete(doctorId)`。
...
### Decision 5: 不改 `services/retire.ts`
...
- `RETIREMENT_LOG`（新）: dirty for `doc-x` PK from creating hook → push retirementLog row ✓

新的 tombstone push 完全靠既有 dirty marker 機制；retire.ts 不需要任何改動。Curator rule: 不改 retire.ts 等於不增加業務邏輯被誤改的風險。

### Decision 6: Achievement / counter integration 不變

`lib/achievement-stats.ts` 跟 `services/counter-backfill.ts` 已經 iterate retirementLog 計 `p1DoctorsRetired` / `derivedTotalDoctors`（= live + retired）等。Cloud-synced 後跨裝置的 achievement evaluation 會更準（裝置 B 看得到裝置 A 的 retire 紀錄）。**不需要任何 achievement-system 改動**。

### Decision 7: Migration vs hotfix worktree 路由

雖然 CLAUDE.md Bug Triage 寫「P1/P2 bug 走 hotfix 常駐 worktree」，本 fix 涉及：

- Supabase migration（新表 + RPC 改）
- Dexie schema bump（v17 → v18）
...
1. `apps/medexam2-hospital-tw/src/lib/sync/account-switch.ts` `clearLocalSyncTables`：表清單加 `db.retirementLog`
2. `apps/medexam2-hospital-tw/src/lib/sync/migration.ts` 內任何 `wipeLocalSyncedTables`-style helper 同步加
3. 新 Supabase migration `0015_delete_my_data_retirement_log.sql`：`CREATE OR REPLACE delete_my_data()`，wipe list 加 `DELETE FROM public.retirement_log WHERE user_id = uid` 一行（placed 在 `hospital_doctors` 旁邊）
...
### Decision 10: 0013 / 0014 / 0015 deploy 順序強制保證

**選擇**：tasks.md 加 explicit owner-step ordering guard：
...
| Cost | Mitigation |
|---|---|
| 加一筆 startup network round-trip | 1 個 RPC call (`upsert_lww('retirement_log', '[]')`) 開銷 < 50ms |
| owner 手動 migration 流程多 2 個 verify step | 已寫進 tasks.md §9，failure 顯而易見 |
...
| **Dexie v18 → v19 migration 失敗** | Migration 是 read-rewrite pattern（read all → dedup → clear → bulkAdd），任何失敗保留 v18 state。dexie 的 `.upgrade()` 失敗會 throw、Dexie 不會 commit；DB 留在 v18 不動，引擎在啟動時報錯，user 看到 sync paused 狀態。 |
...
| **既有 wipe path 漏 retirementLog 導致跨帳號污染**（codex missed scope） | Decision 9 同步擴 clearLocalSyncTables + migration `0015_delete_my_data_retirement_log.sql`；FK `ON DELETE CASCADE` 保留覆蓋 user 完全刪除。 |
| **race: device A 跟 device B 同時 retire 不同 doctor** | retirementLog pk = doctorId，兩筆獨立 row，正常 LWW upsert，無衝突。 |
| **race: device A 跟 device B 對同一 doctor 都 retire（不太可能 — retire 是 destructive，但 offline-since-recruit 的 stale device 理論可能）** | dedup by doctorId（pk），LWW 留下後寫的那筆。`retiredAt` 用兩筆中後寫的；refund 已分別在各裝置 local 結算到 revenue（會 sync race 在 gameCounters 層解）。不會多扣或多退 refund — refund 不再 double-process 因為 Dexie tx 同步 idempotency 由 doctor row 是否存在來把關。 |
...
1. Apply Supabase migration 0013 + 0014 在 dashboard SQL editor（或 `supabase db push` 若有設）。
2. Verify：`SELECT count(*) FROM retirement_log;` 應為 0；`SELECT upsert_lww('retirement_log', '[]'::jsonb);` 應回 0（whitelist 通過）。

**Deploy（CI 自動）**：

3. Merge change → main → CI deploy GitHub Pages + Cloudflare Pages。
4. Active users 自動拿到新 build；第一次 sync cycle 走 v18 Dexie migration + v4 bundle write。

**Per-user 自動修復**（無玩家動作）：

5. v4 client 第一次跑 `pullAllNow({force:true})` → 拉 retirementLog rows → reconcile 清掉 ghost doctors → next push 上傳本地 retirementLog 到 cloud（如果是第一次裝 v4）。
6. 既有殭屍 doctor row 在 cloud `hospital_doctors` 留著，但所有 v4 client 透過 carve-out skip 不應用 — 對 user 視覺不可見。

**Rollback strategy**：

- 程式碼層 rollback：revert PR、CI 重 deploy 上一版。Active session 拿回 v3 build → Dexie v17 → ⚠️ **不能 downgrade**，Dexie 是 monotonic version，v18 → v17 會 throw `VersionError`。
...
- Supabase migration 0013/0014 可獨立 rollback（drop table + revert upsert_lww 到 0012 版本），但若 client 還在跑 v4，會看到 push 失敗 → sync 進 offline 狀態。

**Smoke checklist（owner 在 staging / 自己 prod account 跑）**：

1. retire 1 個 doctor → revenue +refund → refresh → doctor 不復活 → revenue 不增加
2. retire 1 個 doctor → 開 incognito 簽入 → doctor 不在 roster
3. 在 dev console 手動 insert 殭屍 doctor row（模擬 pre-fix 中招）→ refresh → reconcile 清掉
4. `globalThis.__hospitalSync.getStatus()` post-retire 應為 `'idle'`，retirementLog dirty marker 清 0
5. `R2` URL inspector 看 m2 bundle 該有 `data.retirement_log` 陣列
