...
- **AND** `db.retirementLog.where({doctorId: 'doc-x'}).count()` SHALL equal exactly 1
...
- **AND** device B's `db.retirementLog` SHALL contain doctor X's tombstone row
...
- **AND** cloud `retirement_log` rows owned by U SHALL be deleted by `delete_my_data()` (extended via migration `0015_delete_my_data_retirement_log.sql`)
