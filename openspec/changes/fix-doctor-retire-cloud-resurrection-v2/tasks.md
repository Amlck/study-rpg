...
- [ ] 10.5 Pre-commit hygiene: per CLAUDE.md "Multi-Agent Git Safety" rule — `git status` + `git diff --cached --name-status` before any `git add`. Explicit per-file `git add path/to/file.ext`, NEVER `git add -A` / `git add .`. Owner explicit confirms scope before any commit.
- [ ] 10.6 Compose commit message template — propose body covering: change name reference, root cause (v1 pk-change Dexie 4.x limit), v2 additive design summary, list of file changes, codex audit findings still addressed (Attack 1 / Attack 3 / missed scope), smoke checklist outcome including the v18→v19 upgrade verify. Block on owner approval per curator rule "Never `git commit` without explicit user confirmation".
