...
### 2026-05-19 23:08 — Phase 3 接手 first session — Task 4.7 二階 happy path PASS

Phase 3 dogfood kickoff session in `~/coding-scratch/study-rpg-m2` (track-m2 worktree). Wired `.env.local` for both apps with `VITE_CLOUD_SYNC_BACKEND=dual` + `READ_BACKEND=supabase` + `SYNC_WORKER_URL=https://study-rpg-sync-worker.tony85314.workers.dev` (一階 created fresh, 二階 appended; both gitignored). Worker `/health` 200 OK from cloud.

**Task 4.7 — 二階 m2 + bookmarks happy path: PASS** (Chrome MCP smoke on localhost:5173). Sign-in → ConflictChooserModal「使用雲端覆蓋本機」 → 立即遷移 → MigrationBanner cleared → 🟢已同步 chip. Evidence captured: R2 OPTIONS preflight 204 to presigned URL `users/2dc5fdd4-c1a6-4e2d-ac1e-6567d8a6f58e/bookmarks.json.gz?X-Amz-Signature=...`. JWT sub-scoping correct, gzip extension correct, presign roundtrip correct. Banner-cleared = R2 PUT succeeded for both m2 and bookmarks bundles (per Task 4.5 detection: banner triggers iff ANY bundle missing).

**Still pending for Task 4.7 full closure** (next Phase 3 session): (a) 一階 m1 happy path via `apps/medexam-tw` dev server (same flow), (b) partial-failure retry case (mid-PUT kill tab → re-sign-in → banner re-renders → second click resumes from missing bundle). Task 4.8 14-day bake awaits 4.7 full closure.

**Bug 1 (must-fix before broader Phase 3 dogfood) — Worker CORS allowlist gap.** `cloudflare/sync-worker/wrangler.jsonc` lists `CORS_ALLOWED_ORIGINS = "https://fireman333.github.io,http://localhost:5173,http://localhost:4173"`. Vite auto-fallback ports (5174 / 5175 / 5176 when prior ports occupied) get **CORS-blocked at preflight**, manifesting as `r2_push_exhausted: Failed to fetch` after 3 retries. Today worked around by `pnpm exec vite --port 5173 --strictPort`. Permanent fix: extend allowlist to common Vite fallback range (5173–5176) AND/OR wildcard `localhost:*` in dev profile; commit + `wrangler deploy`. Captured as TaskList #4 follow-up.

**Bug 2 (log spam, not blocker) — `targeted_ticket_history` DexieError2.** Console emitted `[sync:push:targeted_ticket_history] DexieError2` during the ConflictChooser「使用雲端覆蓋本機」 path on port 5176 (pre-CORS-fix attempt). Did **not** recur on clean 5173 dual-write run after `使用雲端覆蓋本機` already executed. Likely stale adapter pointer or table reference removed from current Dexie schema. Independent of R2 migration; needs grep across `packages/core` + apps to confirm whether the table still exists in schema or is a legacy reference. Captured as TaskList #5 follow-up.

**Other findings:** (1) `__hospitalSync` debug handle is exposed on 二階 (mirrors 一階's `__sync`) — useful for future smoke sessions. (2) `read_network_requests` Chrome MCP tool only captures requests after first invocation; for live migration verification, call it BEFORE clicking 立即遷移, not after. (3) GH repo Secrets currently only has `VITE_SUPABASE_URL` + `VITE_SUPABASE_ANON_KEY` — no R2 backend flags yet, so prod stays Phase 0 default until Phase 3 flag flip. (4) Working tree this session: 0 tracked-file changes (only gitignored `.env.local` files modified). Other dirty paths (`content/medexam2-tw/*.json`, archive tasks.md, `supabase/functions/`) belong to sibling sessions per prior handoff.

### 2026-05-20 07:11 — Phase 3 session 2 — Task 4.7 一階 m1 closure with caveats + 2 bugs found

Phase 3 dogfood session 2 in `~/coding-scratch/study-rpg-m2` (track-m2 worktree). Continued Task 4.7 from session 1's 二階 PASS to attempt 一階 m1 happy path via `apps/medexam-tw` dev server on `localhost:5173 --strictPort`.

**Task 4.7 一階 — m1 dual-write code path VERIFIED, MigrationBanner not exercise-able with owner account.** Sign-in via Google OAuth (`tony85314@gmail.com` → user `2dc5fdd4-c1a6-4e2d-ac1e-6567d8a6f58e`) → engine started, status `idle` → 🟢已同步 chip → **no MigrationBanner**. Investigation via `detectAllBundlesMigrationNeeded` returned `{m1: needsMigration:false, m2:false, bookmarks:false}`. Root cause: owner Supabase has **0 M1 rows** (all 4 M1 tables `player_state`/`srs_cards`/`item_instances`/`mentor_backlog` empty for this uid) — owner never had M4-era 一階 cloud sync data, only 二階 (counts: `hospital_state:1, hospital_doctors:3, hospital_mastery:14, hospital_question_history:12, question_bookmarks:8`). Banner is **correctly hidden** per spec — it triggers only when Supabase has rows but R2 lacks blob; for users without legacy Supabase data, dual-write seeds R2 directly with no migration ceremony. This is correct UX, not a bug. The underlying m1 dual-write CODE path was still verified end-to-end by manually invoking `__sync.pushAllNow()`: real 2378-byte gzip snapshot landed in R2 with ETag `9464a567…`, gunzip-readable round-trip with `data.player_state[1] + srs_cards[108] + item_instances[10] + mentor_backlog[1]` matching Dexie state.

**★ Bug 3 (high impact, must-fix-before-Phase-4) — Phase 0 task 2.11 smoke blob never deleted.** The R2 key `users/2dc5fdd4-…/m1-snapshot.json.gz` contained a 1-byte `0x74` garbage with ETag `e358efa489f58062f10dd7316b65649e` — the exact byte from the Phase 0 manual JWT smoke test (tasks.md line 2.11). `tasks.md` claims «Test blob deleted after verification» but the blob persisted in R2 for ~1 month, blocking owner's m1 dual-write for the last 3 days (every push attempted `If-None-Match:*` → 412 → recovery loop crash, see Bug 4). Manually overwritten during this session via direct `fetch(presignUrl, { method:'PUT', headers:{Content-Type:'application/gzip'}, body: gz })` (no precondition headers) → 200 OK, new ETag `62af4470…`. **Fix scope**: (a) correct tasks.md 2.11 description to «smoke blob LEFT in R2; cleanup deferred to Bug 3 follow-up change», (b) add Phase 0 cleanup checklist «verify smoke blob deleted via `wrangler r2 object delete study-rpg-saves users/<uid>/<bundle>-snapshot.json.gz` after first smoke; capture with `wrangler r2 object list study-rpg-saves --prefix users/`», (c) consider general teardown discipline in future smoke tasks.

**★ Bug 4 (high impact, must-fix) — engine `pushBundle` 412 recovery loop crashes when existing R2 blob can't be gunzipped.** Flow: first push (no local etag) → PUT `If-None-Match:*` → R2 returns 412 PreconditionFailed (blob exists) → branch into `if (res.status === 412)` → `pullBundle({conditional:false})` → fetch 200 OK + 1-byte body → `gunzipBundle(blob)` throws on invalid gzip → outer `catch (err)` in `pushBundle` catches it → `isUnrecoverable(err)` returns false (only matches `presign_*` patterns) → retry → same crash → after 3 attempts throws `r2_push_exhausted: Failed to fetch`. The error message is **misleading**: «Failed to fetch» suggests a network failure, but the actual fetch succeeded — it's the gunzip decode that crashed, with the error message happening to contain «Failed to fetch» from elsewhere in the stack. **Fix scope** (recommended approach): in `engine-r2.ts.pullBundle`, wrap `gunzipBundle(blob)` in try/catch; on decode failure, still `setEtag(bundle, res.headers.get('ETag'))` from response headers and return `{blobMissing: false, applied: null, etag, decodeFailed: true}`. Then `pushBundle`'s 412 branch checks the etag and uses `If-Match:<etag>` on next iteration to overwrite the corrupt blob. Alternative: in `pushBundle`, if 412 recovery's pull yields no usable etag or decode fails, fallback to no-precondition PUT (riskier — loses LWW guarantee for that push but recovers from corruption). Either way the misleading error message «r2_push_exhausted: Failed to fetch» should be replaced with `r2_blob_corrupt_overwrote` or `r2_push_recovered_via_overwrite` when this path triggers.

**Closure decision per user**: Task 4.7 marked PASS-with-caveats in tasks.md (line 59). Bug 3 + Bug 4 **deferred to a follow-up change** (`fix-r2-engine-recovery-and-cleanup-phase-0-smoke` — name TBD by next session) rather than blocking Phase 3. Phase 3 read cutover still blocked on Task 4.8 14-day bake; bake can now proceed since the corrupted m1 blob is replaced with valid gzip. Partial-failure retry case (Task 4.7 sub-task b) skipped this session — it depends on banner triggering, which depends on Supabase rows existing; not exercise-able with owner account without fixture seeding. Will be naturally covered when first M4-era user (if any) signs in post-Phase-3-flip.
