...
export class HospitalDB extends Dexie {
  ...
  constructor(name = 'study-rpg-medexam2-hospital-tw') {
    ...
    // v19: rewire-hospital-events-to-non-reading-trigger — move hospital event
    ...
    // schema_version stays at 3 — new field is optional/additive on the
    // bundle side; v18-aware clients tolerate the extra key (forward-compat).
    //
    // Note: this lands at v19 directly on top of v18. The
    // `fix-doctor-retire-cloud-resurrection` change (originally tagged v19
    // for retirementLog pk rekey) was reverted on main in commit 99eac9b
    // due to the Dexie 4.x "Not yet support for changing primary key"
    // upgrade error. The v19 slot is reused here; retirementLog stays at
    // the v18 ++id pk and will be rewired through an index-based lookup
    // in a future AAD redesign change.
    this.version(19)
      .stores({
        affinity: '&subjectId',
        doctors: '&id, subjectId, rarity, obtainedAt',
