...
export function StudySessionPage() {
  ...
  const equippedItemMap = useMemo(() => buildEquippedDoctorEquipmentMap(allDoctorEquipment), [allDoctorEquipment])
  ...
  const totalThroughput = useMemo(() => {
    ...
    for (const room of rooms) {
      ...
      t += computeThroughput(room, doctor, getDoctorEquipmentBonus(equippedItem, room.type))
    }
    return t
        ...
        {assignedRooms.length === 0 ? (
          ...
          <ul className="study-session__room-list">
            {assignedRooms.map((room) => {
              ...
              const throughput = computeThroughput(room, d, getDoctorEquipmentBonus(equippedItem, room.type))
              return (
