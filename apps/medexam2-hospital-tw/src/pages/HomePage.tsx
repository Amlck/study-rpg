...
export function HomePage() {
  ...
  const doctorEquipmentTicketsRow = useLiveQuery(() => db.doctorEquipmentTickets.get('global'), [])
  ...
  const allDoctorEquipment = useLiveQuery(() => db.doctorEquipment.toArray(), []) ?? []
    ...
    <main className="app-shell">
      <header className="app-header">
        ...
        <div className="app-header__meta">
          ...
          <Link to="/doctor-equipment" className="nav-link">
            醫師器材 →
          </Link>
      ...
      <div className="ticket-counter-row">
        ...
        <span className="ticket-counter" title="醫師器材補給池使用的器材券">
          🧰 {doctorEquipmentTicketsRow?.available ?? 0} / {DOCTOR_EQUIPMENT_TICKET_CAP}
        </span>
      </div>
      ...
      {(() => {
        ...
        for (const room of rooms) {
          ...
          throughput += computeThroughput(room, d, getDoctorEquipmentBonus(equippedItem, room.type))
        }
