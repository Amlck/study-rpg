...
const SECTIONS: ReadonlyArray<AccordionSection> = Object.freeze([
  ...
  },
  {
    id: 'srs-modifiers',
    icon: '✨',
    title: 'SRS 排程 + 「太簡單」/「我亂猜的」opt-in 按鈕',
    body: [
      '答對的題會被排到複習隊列：第一次答對 3 天後再考、第二次答對 7 天後再考、之後每次間隔 × ease 加倍（過去是第一次 1 天）。答錯的題明天回來。',
      '答對之後可以選擇按底部兩顆 opt-in 按鈕（不按就走預設排程）：✨ 「太簡單」= 下次間隔 ×3、ease ×1.5、並且從「❌ 錯題 / 歷史曾錯」自動移除（明示「我已經會了」）。🤔 「我亂猜的」= 強制明天再考一次驗證（ease 不扣、不算 lapse、獎勵照發）。',
      '兩顆按鈕只在答對狀態出現、答錯時隱藏。三狀態（預設 / 太簡單 / 我亂猜的）— 同一題答對後可切換選擇（按 太簡單 後改按 我亂猜的 = 後者覆蓋）；再點同一顆已 active 的按鈕會取消、SRS 回到預設排程。',
    ],
  },
  {
    id: 'recruitment',
  ...
  },
  {
    ...
    title: '鍵盤快捷鍵（答題系統）',
    body: [
      '題目階段：按 1 / 2 / 3 / 4 highlight A / B / C / D 選項（可改選），按 Enter 送出答案。',
      '答題後階段：1 = ⭐ 收藏切換、2 = ✨ 太簡單（SRS 間隔 ×3、ease ×1.5）、3 = 🤔 我亂猜的（強制明天再考）、Enter = 下一題。三顆 modifier 鍵都是 toggle，按完還要 Enter 才推進。',
      '捲動兩階段共用：Space 翻下一頁、Shift+Space 翻上一頁、↓↑ 微捲、Home / End 跳到題目頂 / 詳解底。',
      '視覺角標只在桌機 / 平板外接鍵盤顯示（觸控裝置自動隱藏）。若 🐞 回報視窗的輸入框聚焦中，所有快捷鍵都會放行給輸入框，不會誤觸。',
    ],
  },
])
