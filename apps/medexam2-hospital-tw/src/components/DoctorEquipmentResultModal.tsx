...
import { DOCTOR_EQUIPMENT_CATEGORY_LABELS, DOCTOR_EQUIPMENT_RARITY_LABELS } from '../data/doctor-equipment'
import { DoctorEquipmentArtwork, hasDoctorEquipmentHeroArt } from './DoctorEquipmentArtwork'
import { describeDoctorEquipment } from '../services/doctor-equipment'
import type { DoctorEquipmentRow } from '../db/schema'
import medicalCaseBodySprite from '../assets/doctor-equipment/medical-case-body.png'
import medicalCaseLidSprite from '../assets/doctor-equipment/medical-case-lid.png'

// entry -> rarity-flash -> silhouette -> revealed -> close
type RevealStep = 'entry' | 'rarity-flash' | 'silhouette' | 'revealed'

interface Props {
  item: DoctorEquipmentRow | null
  wasPity: boolean
  onClose: () => void
}
...
interface OpenBoxProps {
  onOpen: () => void
  rarity: DoctorEquipmentRow['rarity']
}
...
export function DoctorEquipmentResultModal({ item, wasPity, onClose }: Props) {
    ...
    <AnimatePresence>
      {item && (
        ...
        >
          ...
          <AnimatePresence>
            {step === 'rarity-flash' && (
              ...
              >
                ...
                >
                  {DOCTOR_EQUIPMENT_RARITY_LABELS[item.rarity]}
                  {wasPity && ' · 保底'}
                </motion.span>
              </motion.div>
            )}
          ...
          {/* ── Steps 2 & 3: DoctorEquipment card ── */}
          <AnimatePresence>
            {(step === 'silhouette' || step === 'revealed') && (
              ...
              >
                ...
                {!hasDoctorEquipmentHeroArt(item) && (
                  <p className="supply-ceremony__category-hint">
                    {DOCTOR_EQUIPMENT_CATEGORY_LABELS[item.category]}
                  </p>
                )}
